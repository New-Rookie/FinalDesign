...
def _worker_block_b(args):
    """Run one (N_total, seed, algorithm) configuration."""
    import torch
    torch.set_num_threads(1)
    n_total, seed, algo_name, estimator_path, n_train, n_eval, n_windows_train, device, counter = args
...
def run_block_b(
    ...
    with ProcessPoolExecutor(max_workers=n_workers,
        ...
        futures = [pool.submit(_worker_block_b, wu) for wu in work_units]
        records = poll_progress(futures, counter, total_steps,
                                f"Block B ({len(work_units)} cfgs, {total_steps} steps)",
                                unit="step")
    ...
    save_block_results("P2", "B", raw_df=df, summary_df=pivot, log_dir=log_dir)
    return pivot
